...
async def create_board(  
    ...
    """
    try:
        new_post = schema.NewPost(writer=writer, title=title, content=content, del_yn=del_yn)
        return board_crud.insert_post(new_post, db)

    except ValueError as ve:
        db.rollback()
        return JSONResponse(status_code=400, content={"message": f"잘못된 요청입니다: {str(ve)}"})    
    except Exception as e:
        db.rollback()
        return JSONResponse(status_code=500, content={"message": f"서버에서 오류가 발생했습니다 {str(e)}. 문제를 해결할 수 없습니다."})


@router.get(path="/read", description="게시판 조회", response_model=List[schema.PostList])
async def read_all_post(db: Session = Depends(get_db)):
    return board_crud.list_all_post(db)

@router.get(path="/read/{post_no}", description="게시판 상세 조회", response_model=schema.Post)
async def read_post(post_no:int,
                    db: Session = Depends(get_db)):
